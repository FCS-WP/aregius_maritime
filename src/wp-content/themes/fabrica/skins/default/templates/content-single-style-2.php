...
<?php
	...
	if ( apply_filters( 'fabrica_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
		...
		ob_end_clean();
		if ( strpos( $fabrica_post_header, 'post_title' ) !== false	|| strpos( $fabrica_post_header, 'post_meta' ) !== false ) {
	...
	// Post content
	$fabrica_share_position = fabrica_array_get_keys_by_value( fabrica_get_theme_option( 'share_position' ) );
	?>
	<div class="post_content post_content_single entry-content<?php
		if ( in_array( 'left', $fabrica_share_position ) ) {
			echo ' post_info_vertical_present' . ( in_array( 'top', $fabrica_share_position ) ? ' post_info_vertical_hide_on_mobile' : '' );
		}
	?>" itemprop="mainEntityOfPage">
		<?php
		if ( in_array( 'left', $fabrica_share_position ) ) {
			?><div class="post_info_vertical<?php
				if ( fabrica_get_theme_option( 'share_fixed' ) > 0 ) {
	...
	do_action( 'fabrica_action_before_post_footer' );
	?>
	<div class="post_footer post_footer_single entry-footer">
		<?php
		fabrica_show_post_pagination();
		if ( is_single() && ! is_attachment() ) {
			fabrica_show_post_footer();
		}
		?>
	</div>
	<?php
	do_action( 'fabrica_action_after_post_footer' );
	?>
</article>
